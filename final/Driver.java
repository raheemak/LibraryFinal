...
public class Driver{
	...
	while (loggedIn){
		...
		if (currentUser.findBook(title,author)>-1)
			...
			library.getBook(bookPos).addUser(currentUser.getUsername());
			System.out.println("---\nYou have successfully checked out '"+title+"'!\n********");
		    }
		    
		    else if (library.getBook(bookPos).getStatus()==true){
		...
		String author=capitalize(scan.nextLine().trim());
		...
		if (userPos>-1){
		    Book returned= library.getBook(libPos);
		    currentUser.removeBook(userPos);
		    /*get next user on queue, check out*/
		    library.setStatusofBook(libPos, true);
		    if (returned.hasNext()){
			User newb= allUsers.getUser(allUsers.findUser(returned.getNext()));
			returned.removeUser();
			library.checkOut(libPos);
			newb.addBook(returned);
		    }
		    System.out.println("--\nYou have successfully returned the book. ");
		}
		else {
		    System.out.println("--\nYou have not checked out this book. To checkout, type 'CHECK OUT'.\n--");
		}
		System.out.println("********");
	    }
